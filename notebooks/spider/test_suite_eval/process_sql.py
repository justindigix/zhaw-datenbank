...
def parse_col(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    for alias in default_tables:
        table = tables_with_alias.get(alias, None)
        if table is None:
            _tables_with_alias = {}
            for k, v in zip(tables_with_alias.keys(), tables_with_alias.values()):
                _tables_with_alias[k.lower()] =  v
            table = _tables_with_alias[alias.lower()]
        cols = [col.lower() for col in schema.schema[table]]
        if tok.lower() in cols:
            # use schema cols name
            # _tok = schema.schema[table][cols.index(tok.lower())]
            # key = table + "." + _tok
            key = table + "." + tok
            if key not in schema.idMap.keys() and key is not None:
                _lower_keys = [_key.lower() for _key in schema.idMap.keys()]
                if key.lower() in _lower_keys:
                    key = key.lower()
            return start_idx+1, schema.idMap[key]
...
def parse_table_unit(toks, start_idx, tables_with_alias, schema):
    ...
    if key is None:
        # try to lower the toks[idx] and all keys in tables_with_alias
        _tables_with_alias = {}
        for k, v in zip(tables_with_alias.keys(), tables_with_alias.values()):
            _tables_with_alias[k.lower()] = v
        key = _tables_with_alias[toks[idx].lower()]
    if key not in schema.idMap.keys() and key is not None:
            _lower_keys = [_key.lower() for _key in schema.idMap.keys()]
            if key.lower() in _lower_keys:
                key = key.lower()
